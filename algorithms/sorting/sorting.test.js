...
describe("Testing Sortings", () => {
    ...
    test("Testing sort", () => {
        expect(heapSort([1, 2, 3, 4])).toStrictEqual([1, 2, 3, 4]);
        expect(heapSort([4, 2, 3, 5])).toStrictEqual([2, 3, 4, 5]);
        expect(heapSort([4, 3, 2, 1])).toStrictEqual([1, 2, 3, 4]);
        expect(heapSort([40, -2, -30, 1])).toStrictEqual([-30, -2, 1, 40]);
    });
});
